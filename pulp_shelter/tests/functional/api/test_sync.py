# coding=utf-8
"""Tests that sync shelter plugin repositories."""
import unittest
...
from pulp_shelter.tests.functional.utils import gen_shelter_remote
from pulp_shelter.tests.functional.utils import set_up_module as setUpModule  # noqa:F401


# Implement sync support before enabling this test.
@unittest.skip("FIXME: plugin writer action required")
class BasicSyncTestCase(unittest.TestCase):
    """Sync repositories with the shelter plugin."""
    ...
    def test_sync(self):
        ...
        body = gen_shelter_remote()
        remote = self.client.post(SHELTER_REMOTE_PATH, body)
        ...
        self.assertDictEqual(get_content_summary(repo), SHELTER_FIXTURE_SUMMARY)
        self.assertDictEqual(get_added_content_summary(repo), SHELTER_FIXTURE_SUMMARY)
    ...
    def test_file_decriptors(self):
        ...
        remote = self.client.post(SHELTER_REMOTE_PATH, gen_shelter_remote())
        self.addCleanup(self.client.delete, remote['_href'])
...
class SyncInvalidURLTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        body = gen_shelter_remote(url="http://i-am-an-invalid-url.com/invalid/")
        remote = client.post(SHELTER_REMOTE_PATH, body)
